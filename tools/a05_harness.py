...
Hardware constraint (documented in docs/coordination/artem/A05-handoff.md): this
machine is a MacBook Air M2 (no CUDA, no Ollama installed), not the pinned "G"
RTX 3070 box. The task's own spec text says explicitly: "не загружай модели на
слабой машине" (do not load models on a weak machine). So this harness never
loads the real Qwen3 GPU encoder or the real qwen3:8b-q4_K_M Ollama model.
Instead it offers two knowledge/generator substitution modes, chosen on the CLI:
...
def _build_knowledge(mode: str, *, knowledge_dir: Path, delay_seconds: float):
    if mode == "controlled":
...
def build_app(
    *,
    ...
    knowledge = _build_knowledge(
        knowledge_mode, knowledge_dir=knowledge_dir, delay_seconds=retrieval_delay
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--knowledge", choices=["controlled", "real_lexical"], default="controlled"
    )
    parser.add_argument(
